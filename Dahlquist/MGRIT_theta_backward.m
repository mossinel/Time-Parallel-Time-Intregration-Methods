function [err]=MGRIT_theta_backward(la,T,MF,N,u0,K)
%%%% Dahlquist equation MGRIT, Backward Euler, \theta method
...
theta=(MF-1)/(2*MF);
% Forward Euler
G_=(1+la*dT*theta*MF)/(1-(1-theta)*MF*la*dT);   
F_=(1-la*dT/MF)^(-MF); 
...
for k=1:K+1
    err(k)=max(abs(u(1:MF:end)-U{k+1}(1:N+1,:))); % compute error
end
