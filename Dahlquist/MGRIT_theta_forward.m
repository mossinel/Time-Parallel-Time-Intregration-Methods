function [err]=MGRIT_theta_forward(la,T,MF,N,u0,K)
%%%% Dahlquist equation MGRIT, Forward Euler, \theta method
%%%% Following "Multigrid reduction in time for chaotic dynamical systems"
%%%% la: parameter
%%%% T: final time 
...
theta=(MF+1)/(2*MF);
% Forward Euler
G_=(1+la*dT*theta*MF)/(1-(1-theta)*MF*la*dT);   % see beginning of paragraph 2.1 for the choice of division
F_=(1+la*dT/MF)^(MF); 
M_=speye(N+1)-diag(G_*ones(N,1),-1);
A_=speye(N+1)-diag(F_*ones(N,1),-1);
f=zeros(N+1,1);
...
for i = 1:K
    ...
    if i==5||i==25||i==50           
        ...
        plot(t,u,'b');  
        hold on
        plot((0:(T/(N)):T),U{i+1},'r--','linewidth',2);
        plot((0:(T/(N)):T),U{i+1},'r--');
...
for k=1:K+1
    err(k)=max(abs(u(1:MF:end)-U{k+1})); % compute error
end
...
legend('MGRIT error','superlinear bound','linear bound')
