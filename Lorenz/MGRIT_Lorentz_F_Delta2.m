function [U,u,err_inf,err_2,Y]=MGRIT_Lorentz_F_Delta2(sigma,r,b,T,MF,N,u0,K,algo,graph,check)
%%%% Lorentz MGRIT F-relaxation, \Delta
%%%% F relaxation
...
% FORWARD EULER
    for k=1:K 
        ...
        for n=1:N
            ...
            % Delta
            Delta{n+1}=DFn{n+1}-SForwardEuler_Delta(df,ff,TT(n),TT(n+1),U{k}(n,:),1,U{k}(n,:));  
            % check
            if strcmp(check,"Y") 
                func=@(tt,uu) uu+dt*ff(tt,uu);
            ...
            % tau
            tau(n+1,:)=Fn{n+1,:}-SForwardEuler(ff,TT(n),TT(n+1),U{k}(n,:),1)-(Delta{n+1}*U{k}(n,:)')';        
        end
        %difference
        for n=1:N
            U{k+1}(n+1,:)=SForwardEuler(ff,TT(n),TT(n+1),U{k+1}(n,:),1)+(Delta{n+1}*U{k+1}(n,:)')'+tau(n+1,:);
        end
    end
    %  F relaxation
    for n=1:N
...
% BACKWARD EULER
    for k=1:K 
        ...
        for n=1:N
            ...
            %phi_c
            temp=SBackwardEuler(df,ff,TT(n),TT(n+1),U{k}(n,:),1);
            % Delta
            Delta{n+1}=DFn{n+1}-SBackwardEuler_Delta(df,ff,TT(n),TT(n+1),U{k}(n,:),1,[U{k}(n,:);temp]);
            % check
            func=@(tt,uu) uu+dt*ff(tt,uu);
            if strcmp(check,"Y")
                [flag(n), difference(n)]=check_jacobian_BE(DFn{n+1},ff,df,MF,U{k}(n,:),TT(n),TT(n+1),0);
            ...
            % tau
            tau(n+1,:)=Fn{n+1,:}-temp-(Delta{n+1}*U{k}(n,:)')';        
        end
    
        for n=1:N
            U{k+1}(n+1,:)=SBackwardEuler(df,ff,TT(n),TT(n+1),U{k+1}(n,:),1)+(Delta{n+1}*U{k+1}(n,:)')'+tau(n+1,:);
        end
    end
    %  F relaxation
    for n=1:N
