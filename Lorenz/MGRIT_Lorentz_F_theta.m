function [U,u,err_inf,err_2,Y]=MGRIT_Lorentz_F_theta(sigma,r,b,T,MF,N,u0,K,algo,graph)
%%%% Lorentz MGRIT F-relaxation with theta method
%%%% F relaxation
...
% initial guess 
for n=1:N 
    Go(n+1,:)=SThetaEuler(df,ff,TT(n),TT(n+1),U{1}(n,:),1,theta);
    U{1}(n+1,:)=Go(n+1,:);  
end
% iteration
if strcmp(algo,"FE")
% FORWARD EULER
    for k=1:K 
        ...
        for n=1:N
            %[~,F{n+1,:}]=ForwardEuler(ff,[TT(n),TT(n+1)-dt],U{k}(n,:),MF-1);
        ...
        for n=1:N
            ...
            % tau
            tau(n+1,:)=Fn{n+1,:}-SThetaEuler(df,ff,TT(n),TT(n+1),U{k}(n,:),1,theta);        
      end
    
        for n=1:N
            U{k+1}(n+1,:)=SThetaEuler(df,ff,TT(n),TT(n+1),U{k+1}(n,:),1,theta)+tau(n+1,:);
        end
    end
    %  F relaxation
    for n=1:N
...
if strcmp(graph,"Y")
    ...
    %
    figure
    semilogy([1:K],err_inf,'Linewidth',2)
    hold on
    semilogy([1:K],err_2,'Linewidth',2)
